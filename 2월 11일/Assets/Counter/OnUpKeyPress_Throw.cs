...
public class OnUpKeyPress_Throw : MonoBehaviour
{
    ...
    void Update()
    {
        leftB = OnKeyPress_MoveGravity.leftButtonFlag;
        rightB = OnKeyPress_MoveGravity.rightButtonFlag;

        int a = OnKeyPress_MoveGravity.badItem;
        if ((Input.GetKey("right") && a == 0) || (Input.GetKey("left") && a == 1))// 만약 오른쪽 키가 눌리면
        { 
            leftFlag = false;
        }
        if ((Input.GetKey("left") && a == 0) || (Input.GetKey("right") && a == 1))// 만약 왼쪽 키가 눌리면
        { 
            leftFlag = true;
        }
        //버튼
        if((leftB && a == 0) || (rightB && a == 1))
        {
            leftFlag = true;
        }
        if ((rightB && a == 0) || (leftB && a == 1))
        {
            leftFlag = false;
        }

        if (Input.GetKey(KeyCode.A))// 만약 위 키가 눌리면
        {
            throwBall();
        }
        else
